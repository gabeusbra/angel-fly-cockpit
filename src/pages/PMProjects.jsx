...
export default function PMProjects() {
  ...
  const [form, setForm] = useState({ name: "", client_name: "", scope_description: "", status: "active", start_date: "", end_date: "", total_budget: "", payment_type: "one-time" });

  useEffect(() => {
    Promise.all([base44.entities.Project.list("-created_date"), base44.entities.Task.list()])
      .then(([p, t]) => { setProjects(p); setTasks(t); setLoading(false); });
  }, []);

  const handleCreate = async () => {
    await base44.entities.Project.create({ ...form, total_budget: form.total_budget ? parseFloat(form.total_budget) : 0 });
    setShowCreate(false);
    setForm({ name: "", client_name: "", scope_description: "", status: "active", start_date: "", end_date: "", total_budget: "", payment_type: "one-time" });
